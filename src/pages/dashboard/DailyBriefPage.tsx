...
import {
  AlertTriangle, Package, Users, FileText,
  Shield, RefreshCw
} from "lucide-react";
...
export default function DailyBriefPage() {
  ...
  const loadBrief = useCallback(async () => {
    ...
    try {
      ...
      setBrief(data);
    } catch (err) { addNotification({ id: crypto.randomUUID(), type: "error", title: t("common.error"), message: t("dashboard.loadError") }); }
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          <h1 className="page-title">{t("dashboard.dailyBrief")}</h1>
          <p className="page-subtitle">{t("dashboard.dailyBriefSubtitle")}</p>
        </div>
        <button onClick={loadBrief} className="btn-ghost flex items-center gap-2"><RefreshCw className="w-4 h-4" /> {t("dashboard.refresh")}</button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="text-center">
          ...
          <p className="text-sm text-surface-400">{t("dashboard.unpaidInvoices")}</p>
        ...
        <Card className="text-center">
          ...
          <p className="text-sm text-surface-400">{t("dashboard.overdueAmount")}</p>
        </Card>
        <Card className="text-center">
          <Package className="w-8 h-8 text-red-400 mx-auto mb-2" />
          <p className="text-3xl font-bold text-white">{brief?.waste_yesterday || 0}</p>
          <p className="text-sm text-surface-400">{t("dashboard.wasteCartonsYesterday")}</p>
        </Card>
        <Card className="text-center">
          ...
          <p className="text-sm text-surface-400">{t("dashboard.daysSinceLastBackup")}</p>
          <Badge variant={brief?.backup_status === 'green' ? 'success' : brief?.backup_status === 'amber' ? 'warning' : 'danger'}>
            {brief?.backup_status === 'green' ? t("dashboard.backupUpToDate") : brief?.backup_status === 'amber' ? t("dashboard.backupNeedsUpdate") : t("dashboard.backupOverdue")}
          </Badge>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card>
          <h3 className="section-title"><Users className="w-5 h-5 text-gold-400" /> {t("dashboard.topOverdueCustomers")}</h3>
          <div className="space-y-3">
            {(brief?.overdue_customers || []).length === 0 ? (
              <p className="text-sm text-surface-500 text-center py-4">{t("dashboard.noOverdueAmounts")}</p>
              ...
              <div key={i} className="flex items-center justify-between py-2 border-b border-surface-700/30 last:border-0">
                ...
                <div className="text-left">
                  ...
                  <span className="text-xs text-surface-400 mr-2">{t("dashboard.days", { days: c.days })}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card>
          <h3 className="section-title"><Package className="w-5 h-5 text-gold-400" /> {t("dashboard.topLowStockItems")}</h3>
          <div className="space-y-3">
            {(brief?.low_stock || []).length === 0 ? (
              <p className="text-sm text-surface-500 text-center py-4">{t("dashboard.stockFull")}</p>
