...
interface ProdTrendPoint {
  date: string;
  good: number;
  waste: number;
}
interface MonthlyProdPoint {
  month: string;
  cartons: number;
  cups: number;
}
...
interface DashboardData {
  ...
  receivables_milli: number;
  payables_milli: number;
  gross_profit_milli: number;
  gross_margin_bps: number;
  due_cheques_milli: number;
  pending_approvals: number;
  sales_trend: TrendPoint[];
  production_trend: ProdTrendPoint[];
  monthly_production: MonthlyProdPoint[];
...
export default function DashboardPage() {
  ...
  const monthlyProd =
    stats?.monthly_production?.map((d) => ({
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard
          ...
          value={formatOMR(stats?.cash_position || 0)}
          subtitle="النقد والبنوك"
          icon={<Banknote className="w-6 h-6" />}
        />
        <StatCard
          title="ذمم العملاء"
          value={formatOMR(stats?.receivables_milli || 0)}
          subtitle={`${formatOMR(stats?.overdue_amount || 0)} متأخر`}
          icon={<TrendingUp className="w-6 h-6" />}
        />
        <StatCard
          title="ذمم الموردين"
          value={formatOMR(stats?.payables_milli || 0)}
          icon={<TrendingDown className="w-6 h-6" />}
        />
        <StatCard
          title="مبيعات الشهر"
          value={formatOMR(stats?.revenue_milli || 0)}
          subtitle={`هامش إجمالي ${((stats?.gross_margin_bps || 0) / 100).toFixed(1)}%`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard
          ...
          icon={
            <Factory
              className="w-6 h-6"
              style={{ color: liveData?.today_total_cartons ? "#fbbf24" : undefined }}
            />
          }
        />
        <StatCard
          title="الهالك اليوم"
          value={`${stats?.waste_today || 0} كرتون`}
          icon={<AlertTriangle className="w-6 h-6" />}
        />
        <StatCard
          title="شيكات تستحق خلال 7 أيام"
          value={formatOMR(stats?.due_cheques_milli || 0)}
          icon={<FileText className="w-6 h-6" />}
        />
        <StatCard
          title="اعتمادات معلقة"
          value={stats?.pending_approvals || 0}
          subtitle={`${stats?.low_stock_count || 0} تنبيه مخزون`}
            ...
            {expensesPie.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    contentStyle={ChartTooltipStyle}
                    formatter={(value: number) => [formatOMR(value * 1000), "المبلغ"]}
                  />
      ...
      {liveData && (
        <Card className="relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              onClick={() => navigate("/live-production")}
              className="text-xs text-brand-400 hover:text-gold-400 transition-colors font-medium"
            >
              فتح الإنتاج المباشر ←
            </button>
          </div>
          <div className="grid grid-cols-4 gap-5">
            <div className="text-center p-4 bg-surface-800/50 rounded-2xl border border-surface-700/30">
              <p className="text-3xl font-bold text-white">
                {liveData.today_total_cartons?.toFixed(0) || 0}
              </p>
            ...
            <div className="text-center p-4 bg-surface-800/50 rounded-2xl border border-amber-500/20">
              <p className="text-3xl font-bold text-amber-400">
                {liveData.morning_shift_cartons?.toFixed(0) || 0}
              </p>
            ...
            <div className="text-center p-4 bg-surface-800/50 rounded-2xl border border-indigo-500/20">
              <p className="text-3xl font-bold text-indigo-400">
                {liveData.evening_shift_cartons?.toFixed(0) || 0}
              </p>
            ...
            <div className="text-center p-4 bg-surface-800/50 rounded-2xl border border-gold-500/20">
              <p className="text-3xl font-bold gradient-text">
                {(liveData.today_total_cups || 0).toLocaleString()}
              </p>
            ...
            <div className="mt-4 p-3 bg-surface-800/30 rounded-xl border border-surface-700/20">
              <p className="text-xs text-surface-400">
                ...
                <span className="text-white font-medium">
                  {liveData.recent_entries[0]?.product_name ||
                    `منتج #${liveData.recent_entries[0]?.product_id}`}
                </span>
                {" — "}
                <span className="text-gold-400 font-mono">
                  {liveData.recent_entries[0]?.cartons_produced?.toFixed(0)} كرتون
                </span>
              </p>
            </div>
          )}
      ...
      <div className="grid grid-cols-5 gap-5">
        {[
          ...
          <button
            ...
            className="card-hover flex flex-col items-center gap-3 py-7 group"
          >
            <div
              className={`w-14 h-14 rounded-2xl ${color === "amber" ? "bg-amber-500/20 border border-amber-500/30" : "bg-brand-800/30 border border-brand-500/20"} flex items-center justify-center group-hover:shadow-glow transition-all`}
            >
              <Icon
                className={`w-7 h-7 ${color === "amber" ? "text-amber-400" : "text-brand-400"} group-hover:text-gold-400 transition-colors`}
              />
            </div>
