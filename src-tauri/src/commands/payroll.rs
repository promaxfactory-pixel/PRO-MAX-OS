use crate::commands::{payroll_adjustments, rbac};
...
pub fn prepare_payroll_run(
    ...
    rbac::require_role(&conn, user_id, &["admin", "accountant", "hr", "manager"])?;
    let tx = conn.transaction()?;
    payroll_adjustments::ensure_schema(&tx)?;

    let (period_start, period_end, status): (String, String, String) = tx
        .query_row(
    ...
    if status_lower != "draft" && status_lower != "prepared" {
        return Err(AppError::validation(
            "لا يمكن إعادة تحضير تشغيلة تم اعتمادها أو دفعها",
        ));
    }
    ...
    if employees.is_empty() {
        return Err(AppError::validation(
            "لا يوجد موظفون نشطون لتحضير الرواتب",
        ));
    }
    ...
    {
        let basic = if basic_configured > 0 {
            basic_configured
        } else {
            salary
        };
        ...
        let overtime_rate: i64 = tx
            .query_row(
                "SELECT CAST(COALESCE(overtime_rate_milli,0) AS INTEGER)
        ...
        let adjustments = payroll_adjustments::employee_adjustments_for_run(
            &tx,
            employee_id,
            &period_start,
            &period_end,
        )?;
        let bonus = adjustments.bonus_milli;
        let deduction = adjustments.deduction_milli;
        let advance_deduction = adjustments.advance_deduction_milli;
        let insurance_deduction = 0_i64;
        let tax_deduction = 0_i64;

        let gross = basic
            .checked_add(allowances)
            .and_then(|v| v.checked_add(overtime_milli))
            .and_then(|v| v.checked_add(bonus))
            .ok_or_else(|| AppError::validation("إجمالي راتب العامل يتجاوز الحد المسموح"))?;
        let deductions = deduction
            .checked_add(advance_deduction)
            .and_then(|v| v.checked_add(insurance_deduction))
            .and_then(|v| v.checked_add(tax_deduction))
            .ok_or_else(|| AppError::validation("إجمالي الخصومات يتجاوز الحد المسموح"))?;
        let net = gross - deductions;
        if net < 0 {
            return Err(AppError::validation(
                "صافي راتب سالب غير مسموح؛ راجع الخصومات والسلف المعتمدة",
            ));
        }
        ...
        total_gross = total_gross
            .checked_add(gross)
            .ok_or_else(|| AppError::validation("إجمالي المسير يتجاوز الحد المسموح"))?;
        total_deductions = total_deductions
            .checked_add(deductions)
            .ok_or_else(|| AppError::validation("إجمالي خصومات المسير يتجاوز الحد المسموح"))?;
        total_net = total_net
            .checked_add(net)
            .ok_or_else(|| AppError::validation("صافي المسير يتجاوز الحد المسموح"))?;
    }
    ...
    tx.commit()?;
    Ok("تم تحضير مسير الرواتب من ملفات الموظفين والأوفر تايم والتسويات والسلف المعتمدة".to_string())
}
...
pub fn approve_payroll_run(
    ...
    let (period_end, status, accrual_journal, gross, deductions, net):
        (String, String, Option<i64>, i64, i64, i64) = tx
        .query_row(
    ...
    if status.to_lowercase() != "prepared" {
        return Err(AppError::validation(
            "يجب تحضير ومراجعة المسير قبل الاعتماد",
        ));
    }
    ...
    // Re-read approved adjustments and current advance balances before approval.
    // If anything changed after preparation, approval is blocked and the user must
    // prepare again. Application and journal posting happen in this same transaction.
    let breakdown = payroll_adjustments::validate_and_apply_run(&tx, run_id, user_id)?;

    let (overtime, insurance_tax): (i64, i64) = tx.query_row(
        "SELECT COALESCE(SUM(overtime_milli),0),
                COALESCE(SUM(insurance_deduction_milli + tax_deduction_milli),0)
         FROM payroll_run_lines WHERE run_id=?1",
        [run_id],
        |r| Ok((r.get(0)?, r.get(1)?)),
    )?;
    let other_payroll = gross - overtime;
    let salary_expense = other_payroll
        .checked_sub(breakdown.absence_deductions_milli)
        .ok_or_else(|| AppError::validation("خصم الغياب أكبر من الأجر الخاضع له"))?;

    let classified_deductions = breakdown
        .absence_deductions_milli
        .checked_add(breakdown.withholding_deductions_milli)
        .and_then(|v| v.checked_add(breakdown.advance_deductions_milli))
        .and_then(|v| v.checked_add(insurance_tax))
        .ok_or_else(|| AppError::validation("تصنيف خصومات المسير يتجاوز الحد المسموح"))?;
    if classified_deductions != deductions {
        return Err(AppError::validation(
            "تصنيف خصومات المسير لا يساوي إجمالي الخصومات؛ أعد تحضير المسير",
        ));
    }
    ...
    if salary_expense > 0 {
        lines.push((
            ...
            salary_expense,
            0,
            Some("رواتب وبدلات بعد خصم الغياب".to_string()),
        ));
    }
    if overtime > 0 {
        lines.push((
            ...
            overtime,
            0,
            Some("عمل إضافي معتمد".to_string()),
        ));
    }
    if breakdown.advance_deductions_milli > 0 {
        lines.push((
            "1320".to_string(),
            0,
            breakdown.advance_deductions_milli,
            Some("تحصيل سلف موظفين من الرواتب".to_string()),
        ));
    }
    let clearing_deductions = breakdown
        .withholding_deductions_milli
        .checked_add(insurance_tax)
        .ok_or_else(|| AppError::validation("خصومات التسوية تتجاوز الحد المسموح"))?;
    if clearing_deductions > 0 {
        lines.push((
            "2255".to_string(),
            0,
            clearing_deductions,
            Some("جزاءات وخصومات أخرى معلقة للتسوية".to_string()),
        ));
    }
    if net > 0 {
        lines.push((
            ...
            0,
            net,
            Some("صافي رواتب مستحقة للعاملين".to_string()),
        ));
    }
    ...
    let journal_id = crate::commands::accounting::post_to_journal(
        ...
        run_id,
        &period_end,
        "إثبات استحقاق الرواتب والتسويات",
        &lines,
        &user_id.to_string(),
    )?;
    ...
    let _ = rbac::log_audit(
        ...
        None,
        Some(&format!(
            "journal={} absence={} withholding={} advances={}",
            journal_id,
            breakdown.absence_deductions_milli,
            breakdown.withholding_deductions_milli,
            breakdown.advance_deductions_milli
        )),
        None,
    );
    tx.commit()?;
    Ok("تم اعتماد المسير وإثبات الرواتب والخصومات وسداد السلف محاسبيًا".to_string())
}
...
pub fn record_payroll_payment(
    ...
    if input.amount_milli <= 0 {
        return Err(AppError::validation(
            "مبلغ الراتب المدفوع يجب أن يكون أكبر من صفر",
        ));
    }
    let tx = conn.transaction()?;
    ...
    if run_status_lower != "approved" && run_status_lower != "partially paid" {
        return Err(AppError::validation(
            "يجب اعتماد مسير الرواتب قبل تسجيل الدفع",
        ));
    }

    let (line_id, net_milli, paid_milli): (i64, i64, i64) = tx
        .query_row(
    ...
    if input.amount_milli > remaining {
        return Err(AppError::validation(
            "المبلغ أكبر من الراتب المتبقي للعامل",
        ));
    }
    ...
    let source_type = input
        ...
        .to_lowercase();
    let source_account = if let Some(code) = input
        .source_account_code
        .clone()
        .filter(|x| !x.trim().is_empty())
    {
        ...
        match source_type.as_str() {
            ...
            "custody" => {
                let pid = input
                    .custody_id
                    .ok_or_else(|| AppError::validation("حدد العهدة الدافعة للراتب"))?;
                tx.query_row(
                    "SELECT COALESCE(NULLIF(trim(account_code),''),'1110')
    ...
    if source_type == "custody" {
        let pid = input
            .custody_id
            .ok_or_else(|| AppError::validation("حدد العهدة"))?;
        let old_balance: i64 = tx
            .query_row(
        ...
        if old_balance < input.amount_milli {
            return Err(AppError::validation(
                "رصيد العهدة غير كافٍ لدفع الراتب",
            ));
        }
        ...
        tx.execute(
            ...
            params![
                format!("{} 12:00:00", input.payment_date),
                pid,
                input.amount_milli,
                new_balance,
                input.reference,
    ...
    let lines = vec![
        (
            ...
            input.amount_milli,
            0,
            Some("تسوية راتب مستحق".to_string()),
        ),
        (
            source_account.clone(),
            0,
            input.amount_milli,
            Some("مصدر دفع الراتب".to_string()),
        ),
    ];
    let journal_id = crate::commands::accounting::post_to_journal(
        &tx,
    ...
    // A recorded bank transfer is evidence of payment, not a declaration that
    // the Oman WPS comparison has passed. WPS remains a separate reconciliation state.
    ...
    let source_id = if source_type == "custody" {
        input.custody_id
    } else {
        input.cashbank_id
    };
    ...
    tx.execute(
        "UPDATE payroll_run_lines
         SET paid_milli=COALESCE(paid_milli,0)+?1 WHERE id=?2",
    ...
    let new_status = if total_paid >= total_net {
        "Paid"
    } else {
        "Partially Paid"
    };
    ...
    let _ = rbac::log_audit(
        ...
        Some(&format!(
            "employee={} source={} wps={}",
            input.employee_id, source_type, wps_status
        )),
        None,
    );
    tx.commit()?;
...
pub fn create_employee_advance(
    ...
    let (source_account, source_id): (String, Option<i64>) = match source_type.as_str() {
        "company" => (
            crate::commands::accounting::resolve_cash_account(
                &tx,
                input.cashbank_id,
                &method,
            )?,
        ...
        "custody" => {
            let pid = input.custody_id.ok_or_else(|| {
                AppError::validation("حدد العهدة التي صُرفت منها السلفة")
            })?;
            let account = tx
                .query_row(
                    "SELECT COALESCE(NULLIF(trim(account_code),''),'1110')
    ...
    if source_type == "custody" {
        ...
        if old_balance < input.amount_milli {
            return Err(AppError::validation(
                "رصيد العهدة غير كافٍ لصرف السلفة",
            ));
        }
        let new_balance = old_balance - input.amount_milli;
        tx.execute(
    ...
    let lines = vec![
        (
            ...
            input.amount_milli,
            0,
            Some(format!("سلفة العامل {}", employee_name)),
        ),
        (
            source_account,
            0,
            input.amount_milli,
            Some("مصدر صرف سلفة الموظف".to_string()),
        ),
    ];
    let journal_id = crate::commands::accounting::post_to_journal(
        &tx,
        ...
        adv_id,
        &input.date,
        &format!("سلفة موظف - {}", employee_name),
        &lines,
        &user_id.to_string(),
    )?;
    tx.execute(
        "UPDATE employee_advances SET journal_id=?1 WHERE id=?2",
        params![journal_id, adv_id],
    )?;
    if let Some(txn_id) = custody_txn_id {
        tx.execute(
            "UPDATE petty_cash_transactions SET journal_id=?1 WHERE id=?2",
            params![journal_id, txn_id],
        )?;
    }
    ...
    let _ = rbac::log_audit(
        ...
        None,
        Some(&format!(
            "employee={} amount={} source={} journal={}",
            input.employee_id, input.amount_milli, source_type, journal_id
        )),
        None,
    );
    tx.commit()?;
