    ...
    conn.execute(
        "INSERT INTO employee_advances
         (employee_id, amount_milli, date, reason, status, remaining_milli,
          deduction_per_payroll_milli, created_by, created_at)
         VALUES(?1,?2,?3,?4,'open',?5,?6,?7,datetime('now'))",
        params![
            input.employee_id, input.amount_milli, input.date, input.reason,
            input.amount_milli, deduction, user_id.to_string()
        ],
    )?;
    let adv_id = conn.last_insert_rowid();
    let _ = rbac::log_audit(
        &conn, Some(user_id), None, "create_employee_advance", "employee_advances",
        Some(adv_id), None, None, None
    );
    Ok(adv_id)
}
