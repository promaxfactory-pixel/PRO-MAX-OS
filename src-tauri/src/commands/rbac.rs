...
#[allow(dead_code)]
pub fn require_role(conn: &rusqlite::Connection, user_id: i64, allowed_roles: &[&str]) -> Result<(), AppError> {
    ...
    if role == "admin" || allowed_roles.iter().any(|r| *r == role) {
        ...
        Err(AppError::permission(format!("Access denied. Required role: {:?}, your role: {}", allowed_roles, role)))
    }
}
